...
let possibleKeys: Array<{ x5c: string, x5t: string }> = [];
...
async function getMatchingKey(
    token: string
): Promise<{ x5c: string; x5t: string } | undefined> {
 ...
 * @returns {*}
 */
const validateJWTToken = async (token: string, verifyOptions: VerifyOptions | null): Promise<any> => {
